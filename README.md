...
| 智能对话 | 多轮对话会话、SSE 流式响应、对话历史、5 类意图识别、4 维度透明评分、会话标题自动生成 | F12-F13 |
...
- **4 维度透明评分**：skill/experience/education/salary 4 维度评分 + overall 综合分 + reason 理由，LLM JSON 模式输出，含两级兜底（LLM 失败回退 rerank_score，JSON 解析失败全 0）
...
用户输入 → 意图识别节点（5 类：chitchat/search/detail/compare/qa）
    ...
    chitchat  search  detail/    qa
              compare  compare
        ↓       ↓       ↓           ↓
    CHITCHAT  检索+精排  检索+精排  QA_PROMPT
    PROMPT    +评分     +评分      直接回答
        ↓       ↓       ↓           ↓
        └───────┴───────┴───────────┘
                    ↓
...
│   │   ├── core/                     # 基础设施层（含 email_templates_seed 预置模板）
...
- **邮箱**：`admin@talentsense.com`（由 `.env` 的 `ADMIN_EMAIL` 配置）
- 密码：`admin123`
- 角色：`admin`
...
| 类型 | 数量 | 说明 |
|------|------|------|
| 后端单元/集成测试 | 237 | 覆盖 API / Services / Core / Agent / Utils 各层（含 users / RBAC / email_template / qa 意图 / 4 维度评分） |
