...
[![Tests](https://img.shields.io/badge/tests-464%20passed-brightgreen.svg)](#测试)
...
- **RAG 三模块增强**：(1) 查询分解 `query_decompose_node` 替代 `filter_extract_node`，LLM 输出 main_query + sub_queries + structured_filters；(2) 多路召回 Route A（主查询 hybrid_search）+ Route B（sub_queries 独立检索）+ Route C（MongoDB 结构化保底）；(3) 上下文压缩 `context_compress_node` 用 LLM 将同一简历的多 chunks 合并为 ≤500 字精炼摘要，`asyncio.gather` 并发执行
...
- **领域同义词扩展**：`required_skills` 硬过滤前对领域词做语义扩展（nlp→[nlp,bert,fasttext,transformer,jieba,...]），避免"nlp相关的"误杀只写了 BERT/Transformer 但没写 NLP 的候选人；具体技术（html/java/python/react）保持严格匹配。领域词映射表见 `app/utils/skill_synonyms.py`
- **三重相关性过滤**：技能硬过滤（required_skills 子串匹配 + 领域扩展）+ 最低分数截断（overall ≥ 40）+ 简历级去重（按 resume_id 保留最高 rerank_score 的 chunk），避免不相关候选人挤占结果
...
用户输入 → 意图识别节点（5 类：chitchat/search/detail/compare/qa）
    ...
    chitchat  search  detail/    qa
              compare  compare
        ↓       ↓       ↓           ↓
    CHITCHAT  查询分解   复用历史   复用历史
    PROMPT    (decompose) candidates candidates
              ↓
        ┌─────┴─────┐
        ↓           ↓
    上下文压缩   多路召回
    (compress)  Route A 主查询
                Route B 子查询
                Route C MongoDB保底
                ↓
            Reranker 精排
...
| 类型 | 数量 | 说明 |
|------|------|------|
| 后端单元/集成测试 | 296 | 覆盖 API / Services / Core / Agent / Utils 各层（含 users / RBAC / email_template / qa 意图 / 4 维度评分 / 同义词扩展 / 技能硬过滤 / 领域同义词扩展 / 对比复用 / 姓名匹配 / 看板扩展 / RAG 查询分解 / 上下文压缩 / 多路召回） |
| 后端 E2E 测试 | 35 | 端到端招聘闭环业务流程（含邮箱登录/模板发送/RAG 增强 E2E） |
...
- [RAG 增强设计（查询分解+多路召回+上下文压缩）](docs/specs/2026-06-29-rag-enhancement-design.md)
- [RAG 增强实施计划](docs/plans/2026-06-29-rag-enhancement.md)
