...
| 模块 | 功能 | 验收点 |
|------|------|--------|
| 认证管理 | 用户登录、Token 刷新、当前用户、登出、注册申请、改密 | F01-F02 |
...
- **管理员自动初始化**：后端启动时若 admin 账号不存在则自动创建（凭据走 `.env`）
...
│   │   ├── services/                 # 业务逻辑层（13 个 service，含 user_service）
│   │   ├── core/                     # 基础设施层
...
其他用户需通过登录页"申请账号"入口提交注册申请，由管理员审批后方可登录；管理员也可在「用户管理」页直接开号。
...
| `ADMIN_EMAIL` | 自动初始化管理员邮箱 | admin@talentsense.local |
...
| 模块 | 前缀 | 主要接口 | 权限 |
|------|------|---------|------|
| 认证 | `/api/v1/auth` | POST `/login` · POST `/refresh` · GET `/me` · POST `/logout` · POST `/register` · PUT `/password` | 公开 / 登录 |
...
| 邮件 | `/api/v1/email` | POST `/send` · GET `/config` · PUT `/config` | 登录 / 配置 admin |
...
| 类型 | 数量 | 说明 |
|------|------|------|
| 后端单元/集成测试 | 181 | 覆盖 API / Services / Core / Agent / Utils 各层（含 users / RBAC） |
