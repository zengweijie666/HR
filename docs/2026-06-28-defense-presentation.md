...
| 技术栈 | FastAPI + Vue3 + MongoDB + Milvus + Redis + MinIO + BGE-M3 + LangGraph |
| 代码规模 | 后端 ~10000 行 Python / 前端 ~8000 行 TypeScript |
| 测试规模 | 402 个测试（后端 269 + 前端 133）全部通过 |
| 版本 | v2.8.0 |
| 答辩时长 | 15-20 分钟 |
...
>
> 接下来用大约 18 分钟，从项目背景、架构设计、三大核心技术、工程质量四个维度展开介绍。

---

### P2 · 目录（20s）

> 答辩分为五个部分：项目背景与目标、系统架构、三大核心技术、工程质量保障、总结与展望。
>
> 其中核心技术部分将重点介绍 AI 检索流水线、Agent 工作流、全栈工程能力这三个方向。
...
> 这是项目工程能力的核心体现 —— **402 个测试全部通过**。
>
> | 类型 | 数量 | 覆盖范围 |
> |------|------|---------|
> | 后端单元/集成测试 | 237 | API/Services/Core/Agent/Utils 各层 |
...
> **项目总结：**
>
> TalentSense HR 实现了一套完整的 AI 智能招聘全栈系统，覆盖招聘全链路 22 个功能点，核心技术包括 Hybrid 向量检索、4 维度透明评分、LangGraph Agent 工作流、全栈 RBAC 权限体系。代码规模约 18000 行，402 个测试全部通过，达到生产级工程质量。
>
> **个人收获：**
>
...
> 4. **部署优化**：Docker Compose 一键部署 + GPU 推理加速
>
> 感谢老师与同学聆听，欢迎提问。
...
> 1) **Milvus 字段命名规范化**：candidate_id 字段直接存 candidate_id，避免 chunk_map 映射；2) **评分权重可配置**：把 0.3/0.3/0.2/0.2 抽到 .env 或数据库，admin 可调；3) **Docker 化部署**：当前需要手动启动 5 个服务（MongoDB/Redis/Milvus/MinIO/后端），Docker Compose 一键启动更友好；4) **前端组件库抽离**：CandidateCard 等组件可抽离为独立 npm 包复用。
